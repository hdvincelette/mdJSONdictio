...
<br />
<br />
The two main components of the tabular data dictionary template are attributes and entry values, each of which are represented as a row. Entry values are collectively referred to as the "domain" of an attribute. It is important to remember that while entry values are associated with a certain attribute, not all attributes have defined entry values. For example, the attribute "Sex" may have three defined entry values ("F","M", and "U"), where as the attribute "Year" does not have defined entry values since an unlimited number of years can be entered.
<br />
<br />
